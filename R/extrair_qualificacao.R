...
extrair_qualificacao <- function(indice){
  ...
  local_prisao <- stringr::str_extract(atestado,"(?<=\\(SEAP\\)\\*\\:).+") %>%
    stringr::str_trim()
  ...
  data_nascimento <- stringr::str_extract(atestado,"(?<=Nascimento:).+(?=Naturalidade)") %>%
    stringr::str_trim() %>%
    lubridate::dmy()
  ...
  naturalidade <- stringr::str_extract(atestado,"(?<=Naturalidade:).+") %>%
    stringr::str_trim()

  nacionalidade <- stringr::str_extract(atestado,"(?<=Nacionalidade:).+?(?=Estado)") %>%
    stringr::str_trim()

  escolaridade <- stringr::str_extract(atestado,"(?<=Escolaridade:).+(?=Profissão)") %>%
    stringr::str_trim()

  profissao <- stringr::str_extract(atestado,"(?<=Profissão:).+") %>%
    stringr::str_trim()

  estado_civil <- stringr::str_extract(atestado,"(?<=Civil:).+") %>%
    stringr::str_trim()

  pena_imposta <- stringr::str_extract(atestado,"(?<=Imposta:).+") %>%
    stringr::str_trim()
